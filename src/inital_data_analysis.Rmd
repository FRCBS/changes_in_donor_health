...
data <- data %>% filter(!is.na(health))  %>% dplyr::select(-health)
data %>% group_by(questionnaire) %>% count()
```
...
```{r}
#Save questions only asked in the second questionaire
...
#Additional questions of the te second questionaire
...
```{r}
ggplot(changedata) + geom_bar(aes(x=life_qual))
```

Could excellent be combined very_good and very_poor and poor to average?
```{r}
changedata <- changedata %>% mutate(life_qual = case_when(
...
```{r}
ggplot(changedata) + geom_bar(aes(x=employment))
```

This is not at all informative. Could it be fulltime or not?

```{r}
changedata <- changedata %>% mutate(employment = case_when(
...
```{r}
ggplot(changedata) + geom_bar(aes(x=education))
```

Also some grouping would be good in here.

```{r}
changedata <- changedata %>% mutate(education = case_when(
  education == "graduate_degree" | education == "bachelor_degree"  ~ "university",
  education == "vocational_school" | education == "trade-school" |  education == "vocational_college" |  education == "high-school"  ~ "vocational",
  education == "basic_education" | education == "middle-school"  ~ "basic"
