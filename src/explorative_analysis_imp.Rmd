...
-reparametrisoi poor-satisfactory improved health malli
...
This file includes the code for computing the regression results regarding the predictors of worsened self-related health during the FinDonor study for the explorative analysis parts. Models containing as many predictor variables as possible are fitted to data and confidence intervals for coefficients computed with bootstrapping.
...
regression_data_men <- regression_data_men %>%  dplyr::select(-smoking_behavior,-health_outcome,-health_very_good,-health_excellent)
regression_data_post_menop_women <- regression_data_post_menop_women %>% dplyr::select(-smoking_behavior,-health_outcome,-health_very_good,-health_excellent)
regression_data_pre_menop_women <- regression_data_pre_menop_women %>%  dplyr::select(-smoking_behavior,-health_outcome,-health_very_good,-health_excellent)
...
logit_pre_menop_women <- glm(health_outcome ~ age + starting_weight + BMI_diff + 
                              ...
                              donation_frequency + 
#                               health_poor + #no variation
#                               health_satisfactory + # separation exists among the sample points.
                               cholesterol_d + 
                              blood_sugar_d +
                               HBP_d +
                                low_hb_hist_d +
...
regressor_values <- c(
 "age" = "Age",
   ...
   "health_satisfactory" ="Initial health rating \n satisfactory vs Good/Very good/Excellent",
...
p2  <- ggplot(aes(x = regressor, y = OR, color = group, fill = group),data = plotdata) +
  ...
  geom_pointrange(data = bootstrap_Bca_CI_2,
                  aes(ymin=Bca_inf, ymax = Bca_sup, shape = is_sig, color = group, fill = group),
                  position = position_dodge(width = 0.7), size = pointrange2size, linetype = 1) +
  # # geom_point(data = bootstrap_Bca_CI, 
  #            aes(shape = is_sig, fill = group),
  #            position = position_dodge(width = 0.8), size = 2) +
    scale_color_manual(values = c(  "#0065BD", "#BA0303", "#C50084" ),
