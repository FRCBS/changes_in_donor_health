...
-sitten kuvasta siisti:
  -inf arvot
  -iso kuva supplementtiin
  -pikku kuva julkaisuun (p-arvo suodatus?)
...
regression_data_men <- regression_data_men %>%  dplyr::select(-smoking_behavior)
regression_data_post_menop_women <- regression_data_post_menop_women %>% dplyr::select(-smoking_behavior)
regression_data_pre_menop_women <- regression_data_pre_menop_women %>%  dplyr::select(-smoking_behavior)
...
#Small plot of filtered variables to main paper
```{r}
...
#Big plot of all variables to supplement
```{r}




p  <- ggplot(aes(x = regressor, y = OR, color = group, fill = group),data = plotdata) +
  geom_hline(yintercept = 1 , color = "grey", linetype = "dotted",
             size = 1) +
  # geom_violin(alpha = 0.15, trim = TRUE, position = position_dodge(width = 0.8), size = 0.1) +
  ...
  #            position = position_dodge(width = 0.8), size = 2) +
    scale_color_manual(values = c(  "#0065BD", "#BA0303", "#C50084" ),
     ...
     scale_fill_manual(values = c(  "#0065BD", "#BA0303", "#C50084" ),
                     limits = c("Men",  "Post-menopausal women","Pre-menopausal women" )) +
 scale_shape_manual(values = c(1,16)) +
  scale_y_log10(limits=c(1/50,50)) +
  # scale_x_discrete(labels=parse(text=unique(bootstrap_Bca_CI$regressor)))
  # ylim(1/35, 35) +
  guides(shape = FALSE) +
  theme_classic() +
