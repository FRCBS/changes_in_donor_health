...
FD_activity_biomarker_data <-
 ...
 #    left_join(first_questionnaire_data %>% 
  ...
  #            by = "donor") %>% 
  # Add donro group
  #  left_join(donor_group_df %>% 
  #              dplyr::select(donor, group),
   #           by = "donor") %>% 
...
regression_data <-
  ...
  inner_join(first_questionnaire_data %>% 
               dplyr::select(donor, QR17),
             by = "donor") %>% 
  mutate(health_very_good = ifelse(QR17 == "Very_good",1,0),
         health_excellent = ifelse(QR17 == "Excellent", 1, 0)) %>% 
...
final_data %>% 
  filter(questionnaire == "First") %>% 
  dplyr::select(donor, QR17,group) %>% 
  inner_join(FD_activity_biomarker_data,
             by = c("donor", "group")) %>% 
  # count(group, QR17, health_evolution) %>% 
  # group_by(group, QR17) %>% 
  ...
  mutate(QR17 = ifelse(QR17 == "Very_good" , "Very good", as.character(QR17)),
        QR17 = ordered(QR17, levels = c(  "Satisfactory", "Good", "Very good", "Excellent") )) %>% 
