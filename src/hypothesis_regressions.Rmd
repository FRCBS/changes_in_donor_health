...
-univariate dists of significant:
  initial weigth
  initial ferritin
  donation frequency
  vs outcome
  
  muriel has a very nice plot for this.
...
regression_data <-
  ...
  dplyr::select(donor, starting_weight,BMI_diff, age, group, log_Ferritin_beginning, Hb_beginning,
                donation_frequency, health_very_good,health_excellent,health_outcome,health_outcome_neg, smoking_behavior,Ferritin_beginning,age_o,starting_weight_o,Hb_beginning_o )
...
table1data <- table1data %>% dplyr::select(-age,-starting_weight,-log_Ferritin_beginning,-health_very_good,-health_excellent,-health_outcome,-health_outcome_neg, -smoking_behavior,-Hb_beginning) %>%
  rename(
  "Age" = age_o,
...
tab3Mat %>% 
  kable()
  write.csv(tab3Mat, file = "../results/table_1_population_20200512.csv")
```
...
regression_data_pre_menop_women <-
  ...
  inner_join(regression_data %>% 
               dplyr::select(donor, health_very_good, health_excellent, health_outcome,health_outcome_neg,smoking_behavior),
             by = "donor")
...
regression_data_men <-
  ...
  spread(key = key, value = value_scale) %>% 
  inner_join(regression_data %>% 
               dplyr::select(donor, health_outcome, health_very_good, health_excellent,health_outcome_neg,smoking_behavior),
             by = "donor")
```
  ...
  labs(title = NULL,
   ...
   theme(axis.title = element_text(size = 14),
        ...
        plot.subtitle = element_text(size = 16,
                                     hjust = 0.5))
   #
ggplot2::ggsave(filename = "../results/figures/log_reg_results_b.pdf",
       width = 35,
       height = 18,
       dpi = "print",
       units = "cm")
