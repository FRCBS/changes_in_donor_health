...
> summary(regression_data$BMI_diff
+ )
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
-9.3499 -0.3165  0.3156  0.4653  1.0767  8.9844 

This needs to be looked at in the original pre_IDA stage
...
```
```{r}
pferd %>% group_by(group, Difference) %>% summarise(mean=mean(Ferritin_beginning),median=median(Ferritin_beginning))
...
pfqd <- final_data %>% 
  ...
  mutate(group = case_when(group == "Pre_menopause_women" ~ "Pre-menopausal \n women",
                           ...
                           TRUE ~ "Men"),
         group = ordered(group, levels = c(  "Men", "Post-menopausal \n women", "Pre-menopausal \n women") )) 
pfq <-  ggplot(pfqd, aes(x = Difference, y = nb_donations_between_questionnaires, color = group)) +
  # geom_hline(yintercept = 15, color = "red", linetype = 3) +
  geom_boxplot(outlier.alpha = 0) +
  geom_beeswarm(alpha = 0.5, shape = 1) +
  # scale_y_log10() +
...
```
```{r}
pfqd %>% group_by(group, Difference) %>% summarise(mean=mean(nb_donations_between_questionnaires),median=median(nb_donations_between_questionnaires))
